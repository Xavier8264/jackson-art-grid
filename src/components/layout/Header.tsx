...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-primary/10 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container flex h-16 items-center gap-4" style={{ ['--nav-offset' as any]: '200px' } as React.CSSProperties }>
        {/* Logo & Branding */}
        ...
        {/* Desktop Navigation - Offsettable (adjust --nav-offset to move) */}
        <nav className="hidden items-center gap-1 md:flex" style={{ marginLeft: 'var(--nav-offset, 250px)' }}>
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
